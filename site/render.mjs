...
function renderOnchain() {
  return [
    '<section id="onchain-products" class="content-section s-onchain" data-formation="3" data-accent="#ff6490">',
      '<div class="shell">',
        '<header class="section-heading">',
          '<p class="eyebrow">2023 — present · shipped onchain products</p>',
          '<h2>Onchain products under real load</h2>',
          '<p>Historical and current systems where real-time gameplay, DAO stewardship, irreversible assets, and chain migrations became product and operations problems—not contract demos.</p>',
        '</header>',
            ...
            '<article class="onchain-project ', product.id, '">',
              ...
              '<div class="onchain-media ', product.media.length > 1 ? 'onchain-media-pair' : '', '">',
                product.media.map((media) => [
                  media.enlarge
                    ? '<button type="button" class="onchain-media-button" data-lightbox-src="' + h(media.src) + '" data-lightbox-alt="' + h(media.alt) + '" data-lightbox-label="' + h(media.label) + '" aria-label="Enlarge ' + h(media.label) + '">'
                    ...
                    '<span>', h(media.label), media.href ? ' ↗' : media.enlarge ? ' ＋' : '', '</span>',
                  media.enlarge ? '</button>' : media.href ? '</a>' : '</figure>',
                ].join('')).join(''),
              '</div>',
              '<nav class="onchain-evidence evidence-count-', product.evidence.length, '" aria-label="', h(product.title), ' evidence">', product.evidence.map(([type, label, href]) => [
                '<a href="', h(href), '"><small>', h(type), '</small><strong>', h(label), ' <span aria-hidden="true">↗</span></strong></a>',
              ].join('')).join(''), '</nav>',
            '</article>',
          ].join('')).join(''),
        '</div>',
        '<dialog id="realm-lightbox" class="realm-lightbox" aria-labelledby="realm-lightbox-caption">',
          '<div class="realm-lightbox-frame">',
            '<button type="button" class="realm-lightbox-close" data-lightbox-close aria-label="Close enlarged image">Close <span aria-hidden="true">×</span></button>',
            '<button type="button" class="realm-lightbox-image" data-lightbox-close aria-label="Close enlarged Realm image">',
              '<img src="/assets/realm-anima-staking.webp" alt="Realm ANIMA staking and rewards interface" width="814" height="798">',
            '</button>',
            '<p id="realm-lightbox-caption">Historical ANIMA staking interface</p>',
          '</div>',
        '</dialog>',
